...
//   const link = `https://meet.google.com/lookup/${id}`;
// console.log(link)
...
app.get('/pharmahomepage', (req, res) => {
  res.render(path.join(__dirname, '/pharmacist/homepage'), { 'session': session.loggedin, 'not_registered': 1, 'invalid': 1, 'alreadyAccount': 1,'message':2 });
});
...
const bookSchema = new mongoose.Schema({
  Id: String,GoogleMeetLink:String, Flag: Boolean, Price: Number, Type: String, UserId: String, DoctorId: String, DoctorName: String, DoctorField: String, TestId: String, TestName: String, TestResult: String, MedicineId: String, MedicineName: String, Date: Date, Quantity: Number
});
...
const docSchema = new mongoose.Schema({
  Id: String, Name: String, Phone: Number, Email: String, Qualification: String, Field: String, Experience: String, Location:String, DOJ: Date
});
...
app.post('/pharmalogin', (req, res) => {
  ...
  pharmacist.findOne({ 'Email': email }, 'Password', (err, user) => {
    if (!user) {
      ...
      // MongoDB query to retrieve the data
      ...
      Booking.find({}, (err, docs) => {
        ...
        docs.forEach((doc) => {
          ...
          if (doc.Flag == false) {
            ...
            data1.push(bookingData)
            console.log("Zero => ", data[0].name)
            message = 1;
          }
        });
        ...
        session.loggedin = true;
        session.email = email;
        res.render(path.join(__dirname, './pharmacist/homepage'), { 'session': session.loggedin, 'not_registered': 0, 'invalid': 0, 'alreadyAccount': 1, data, message });
      });

    } else if (user.Password != psw) {
      res.render(path.join(__dirname, './pharmacist/homepage'), { 'session': session.loggedin, 'not_registered': 0, 'invalid': 1, 'alreadyAccount': 1,'message':1 });
    }
  });
});
  ...
  // const book1 = new Booking();
    // for(var i = 0; i<data1.length;i++){
      ...
      Booking.find({}, (err, docs) => {
        ...
        // var message = 0;
        var data = data1
        res.render(path.join(__dirname, './pharmacist/homepage'), { 'session': session.loggedin, 'not_registered': 0, 'invalid': 0, 'alreadyAccount': 1, data, 'message':0 });
      // if(i!=data1.length-1)
      // doc.Flag=false;
      }
    })
...
app.post('/Updatemedicine', (req, res) => {
  if (!session.loggedin) {
    ...
    console.log(price)
    med.save();
    res.render(path.join(__dirname, './pharmacist/homepage.ejs'), { 'session': session.loggedin, 'not_registered': 0, 'invalid': 0, 'alreadyAccount': 1,'message':2 });
  }
});
var flag_u = 1;
localStorage.setItem(flag_u);
...
app.post('/submitAddress', (req, res) => {
  ...
  address.save().then(() => {
    ...
    res.render(path.join(__dirname, 'payment'), {
...
app.post('/consult_booking', (req, res) => {
  ...
  if (!usrId) {
   ...
   // Create the Google Meet link
   const link = `https://meet.google.com/lookup/${id}`;
      console.log(doctorId+" "+doc1.Name+"  "+doc1.Field);
      book.DoctorName = doc1.Name;
      book.DoctorField = doc1.Field;
      book.GoogleMeetLink = link;
      book.save().then(() => {
        User.findOne({ 'Id': usrId }, (err, user) => {
          ...
          const htmlcont = `
          <body>
            <h2>Doctor Consultation Booking Confirmation</h2>
            <p>Dear ${user.Name},</p>
            <p>We're pleased to confirm your booking consultation with <b> ${doc1.Name} </b> on <b> ${date} </b> at <b> ${time} </b>.</p>
            <p>The consultation will be held at our clinic located at <b>${doc1.Location} </b>. Please arrive 10 minutes before your scheduled appointment to complete any necessary paperwork.</p>
            <p> If you want to online consultation join this google meet link on selected time <b> ${link} </b> </p>
            <p>If you need to reschedule or cancel your appointment, please let us know at least 24 hours in advance so we can accommodate other patients.</p>
            <p>If you have any questions or concerns, please don't hesitate to contact us at <b>+91 9571020367 </b> or reply to this email.</p>
            <p>Thank you for choosing our clinic for your healthcare needs. We look forward to seeing you soon.</p>
            <br>
            <p>Best regards,</p>
            <p>Team Medicinewala</p>
          </body>
          `
...
app.post('/test_booking', (req, res) => {
  ...
  if (!usrId) {
   ...
   const b_id = "LT" + usrId;
    ...
    //     if(i==0){alltest  = test.Name;}
    //     else {alltest  = alltest + ","+test.Name;}
    //     console.log(alltest)
    //     book.save();
    //   })
    // }
...
for (var i = 0; i < arr.length; i++) {
  ...
  promises.push(Test.findOne({ 'Id': arr[i] }).exec().then((test) => {
    const book = new Booking();
    book.Id = b_id; book.Type = "Lab Test"; book.UserId = usrId; book.TestId = test.Id; book.TestName = test.Name; book.Date = date;
    if(i==0){alltest  = test.Name;}
    else {alltest  = alltest + ","+test.Name;}
...
Promise.all(promises).then(() => {
    ...
    <p>Dear ${user.Name},</p>
    <p>You have requested a <strong> ${alltest} </strong> lab test through our web app. We hope this message finds you well.</p>
    <p>We are pleased to confirm that your lab test has been scheduled for <strong> ${date} </strong> at <strong> ${time} </strong> as requested. Our health staff will collect the sample at the designated time, so we kindly ask that you make yourself available and ready for the test.</p>
    <p>Please note that it is important to follow the instructions given to you before the test. You may need to fast for a certain period or avoid certain foods or drinks before the test. If you have any questions or concerns, please do not hesitate to contact us.</p>
    <p>Once your lab test has been completed, we will send you the results via email. You can expect to receive your results within <strong> 1 Day </strong>. In the meantime, if you have any further questions or concerns, please do not hesitate to contact us.</p>
    <p>Thank you for choosing our web app for your lab test needs. We appreciate your business and look forward to serving you.</p>
    <p>Best regards,</p>
    <p>Team Medicinewala</p>
  </body>
    `
...
app.post('/disease_prediction', (req, res) => {
  ...
  const pythonProcess = spawn('python', ["./predict/predict.py", final_list]);
